...
export default function KeywordList({
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        const result = await fetchKeywords({
          ...
          limit: pageSize,
          sort,
          minSearchVolume: minSearchVolume || null,
          search,
          tiers,
        });
    ...
    const timeoutId = setTimeout(loadInitialData, 800); // 800ms debounce for search
  ...
  const HeaderCell = ({
    ...
    <div
      className={`${width} shrink-0 px-2 py-3 text-${align} font-semibold text-zinc-500 uppercase tracking-wider text-xs sm:text-sm bg-zinc-50 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 ${className}`}
    >
      {label}
    </div>
  );
  ...
  const DataCell = ({
    ...
    <div
      className={`${width} shrink-0 px-2 py-2 text-${align} tabular-nums text-zinc-600 dark:text-zinc-400 truncate text-xs sm:text-sm ${className}`}
      title={title}
    >
      {value}
    </div>
        ...
        <div className="min-w-max">
          ...
          <div className="flex items-center bg-zinc-50 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 sticky top-0 z-20">
            <HeaderCell
              ...
              align="left"
              className="sticky left-0 z-30 shadow-[1px_0_0_0_rgba(0,0,0,0.1)] dark:shadow-[1px_0_0_0_rgba(255,255,255,0.1)]"
            />
            ...
            {keywords.length === 0 ? (
              ...
              keywords.map((post) => (
                ...
                >
                  ...
                  />
                  <div
                    className={`${colWidths.tier} flex justify-center shrink-0`}
                  >
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-bold 
                                            ${
                                              post.tier === "PLATINUM"
                                                ? "bg-cyan-100 text-cyan-700 dark:bg-cyan-900 dark:text-cyan-300 ring-1 ring-cyan-200 dark:ring-cyan-800"
                                                : post.tier === "GOLD"
                  ...
                  <DataCell
                    value={post.total_search_cnt?.toLocaleString()}
                    width={colWidths.search}
                  />
                  <DataCell
                    value={
                      typeof post.golden_ratio === "number"
                  ...
                  />
                  <DataCell
                    value={post.blog_doc_cnt?.toLocaleString()}
                    width={colWidths.doc}
                  />
                  <DataCell
                    value={post.cafe_doc_cnt?.toLocaleString()}
                    width={colWidths.doc}
                  />
                  <DataCell
                    value={post.web_doc_cnt?.toLocaleString()}
                    width={colWidths.doc}
                  />
                  <DataCell
                    value={post.news_doc_cnt?.toLocaleString()}
                    width={colWidths.doc}
                  />
                  <DataCell
                    value={post.pc_search_cnt?.toLocaleString()}
                    width={colWidths.search}
                  />
                  <DataCell
                    value={post.mo_search_cnt?.toLocaleString()}
                    width={colWidths.search}
                  />
                  <DataCell
                    value={post.pc_click_cnt?.toLocaleString()}
                    width={colWidths.click}
                  />
                  <DataCell
                    value={post.mo_click_cnt?.toLocaleString()}
                    width={colWidths.click}
                  />
                  <DataCell
                    value={
                      typeof post.pc_ctr === "number"
                        ? `${post.pc_ctr.toFixed(2)}%`
                        : ""
                    }
                    width={colWidths.ctr}
                  />
                  <DataCell
                    value={
                      typeof post.mo_ctr === "number"
                        ? `${post.mo_ctr.toFixed(2)}%`
                        : ""
                    }
                    width={colWidths.ctr}
                  />
                  <DataCell
                    value={post.pl_avg_depth}
                    width={colWidths.comp}
                    title="평균 광고 노출 수"
                  />
                  <DataCell
                    value={post.comp_idx}
